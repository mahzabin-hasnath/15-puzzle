...
function moveTile(tile, row, column) {
  ...
  if(column > 0 && boardArray[row][column - 1] == 0) {
    dx = -1; // move left
  } else if(column < length && boardArray[row][column + 1] == 0) {
    dx = 1; // move right
  } else if(row > 0 && boardArray[row - 1][column] == 0) {
    dy = -1; // move up
  } else if(row < length && boardArray[row + 1][column] == 0) {
    dy = 1; // move down
  } else {
    return;
  }
  ...
  boardArray[row + dy][column + dx] = value;
